...
#include <cublasLt.h> // Include cuBLASLt header
#include <cuda_fp16.h>
#include <iostream>
#include <vector>
#include <iomanip>
...
int main() {
    ...
    CHECK_CUBLAS(cublasLtMatrixLayoutCreate(&matA_fp32, CUDA_R_32F, K, M, K)); // leading dimension is K
    CHECK_CUBLAS(cublasLtMatrixLayoutCreate(&matB_fp32, CUDA_R_32F, N, K, N)); // leading dimension is N
    CHECK_CUBLAS(cublasLtMatrixLayoutCreate(&matC_fp32, CUDA_R_32F, N, M, N)); // leading dimension is N
    ...
    // half alpha and beta (IMPORTANT TO CAST)
