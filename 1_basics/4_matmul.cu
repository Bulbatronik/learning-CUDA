...
// Flip column of B and apply dot product
...
void matmul_cpu(float *A, float *B, float *C, int m, int k, int n) {
    // IN THE MEMORY IT IS LIKE AN ARRAY (LAYOUT)
    for (int i = 0; i < m; i++) { // row of A
        for (int j = 0; j < n; j++) { // column of B 
            float sum = 0.0f;
            for (int l = 0; l < k; l++) { // column of A and row of B
                // Use wrapping and offset to access elements
                sum += A[i * k + l] * B[l * n + j];
            }
            C[i * n + j] = sum;
        }
...
__global__ void matmul_gpu(float *A, float *B, float *C, int m, int k, int n) {
    // Which threads do we want in a vertical and horizontal direction
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;

    if (row < m && col < n) { // check bounds
        float sum = 0.0f;
        for (int l = 0; l < k; l++) {
            // Use wrapping and offset to access elements
