...
int main() {
    ...
    // Copy data to device from the host (dst, src, how much, type)
    cudaMemcpy(d_a, h_a, size, cudaMemcpyHostToDevice);
    cudaMemcpy(d_b, h_b, size, cudaMemcpyHostToDevice);

    // Define grid and block dimensions
    int num_blocks = (N + BLOCK_SIZE - 1) / BLOCK_SIZE; // Volumetric but is laid out in a 1D array
    // BLOCK_SIZE - # threads in one block; N - # elements -> calculate how to split elements among blocks
    // N = 1024, BLOCK_SIZE = 256, num_blocks = 4
    // (N + BLOCK_SIZE - 1) / BLOCK_SIZE = ( (1025 + 256 - 1) / 256 ) = 1280 / 256 = 4 rounded (floor)
    ...
    // Verify results (Result GPU == Result CPU)
