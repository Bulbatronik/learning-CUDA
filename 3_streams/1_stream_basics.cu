...
#define CHECK_CUDA_ERROR(val) check((val), #val, __FILE__, __LINE__) // macro to check CUDA errors
...
int main(void) {
    ...
    float *h_A, *h_B, *h_C;
    float *d_A, *d_B, *d_C;
    cudaStream_t stream1, stream2; // declare streams
    ...
    CHECK_CUDA_ERROR(cudaMalloc((void **)&d_A, size)); // make sure it ran successfully
    ...
    // Copy inputs to device asynchronously (copy at the same time)
