...
int main(void) {
    ...
    // Make stream2 wait for event (wait for stream1 to finish)
    ...
    // Synchronize streams (wait for both of them to finish)
    //CHECK_CUDA_ERROR(cudaStreamSynchronize(stream1)); // Not needed, because of line 66
