...
#if __CUDA_ARCH__ >= 530 // if the architecture is greater than 530, use the __hmul2 intrinsic
    for (int i = idx_x; i < size; i+=stride) {
        dual_z[i] = __half22float2(__hmul2(dual_y[i], dual_x[i]));
    }
#else // if the architecture is less than 530, use the __half22float2 intrinsic
    for (int i = idx_x; i < size; i+=stride) {
        dual_z[i] = __half22float2(dual_x[i]) * __half22float2(dual_y[i]);
    }
#endif // if the architecture is greater than 530, use the __hmul2 intrinsic
}

void fhma_host(half *h_x, half *h_y, float *h_z, int size)
{
    #pragma omp parallel // use openmp to parallelize the loop
    {
    #pragma omp for // use openmp to parallelize the loop
