...
#define N 2048 // number rows or columns in the matrix
#define BLOCK_SIZE 32 // 32 x 32 matrix of threads in a block (max 1024 threads per block)
...
__global__ void matrix_transpose_naive(int *input, int *output) {
	...
	int indexX = threadIdx.x + blockIdx.x * blockDim.x; //column index
	int indexY = threadIdx.y + blockIdx.y * blockDim.y; //row index
	int index = indexY * N + indexX; // read from input at (row, col)
	int transposedIndex = indexX * N + indexY; // write to output at swapped (row, col)
