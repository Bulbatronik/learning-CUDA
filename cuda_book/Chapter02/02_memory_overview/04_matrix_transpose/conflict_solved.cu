...
__global__ void matrix_transpose_shared(int *input, int *output) {
	// Without it, when threads read sharedMemory[localIndexY][localIndexX] with swapped 
	// indices, many threads can hit the same shared memory bank at once (bank conflict).
